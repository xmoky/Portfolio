 ...
 * @author Jaime
 */
@WebServlet(name = "JuegosConsola", urlPatterns = {"/juegosConsola"})
public class JuegosConsola extends HttpServlet {
        ...
        if (session != null) {
            ...
            if ("usuario".equals(tipoUsuario)) {
                try ( PrintWriter out = response.getWriter()) {
                    ...
                    out.println("<title>Servlet JuegosConsola</title>");
                    out.println("</head>");
                    out.println("<body>");
                    ...
                    try ( PreparedStatement elStatementPreparado = laConexion.prepareStatement(
                            "SELECT * FROM juegos inner join consolas on juegos.idConsola=consolas.idConsola where nombreConsola like ?")) {
                        elStatementPreparado.setString(1, nombreConsola);
